    ...
    if($req->rowCount() == 1) {
        ...
        if(isset($_POST['valider'])) {
            ...
            if(!empty($titre) AND !empty($date) AND !empty($description) AND !empty($code_color) AND !empty($img_illustration)) {
                $req = $conn->prepare('UPDATE albums SET title = ?, date = ?, description = ?, code_color = ?, img_illustration = ? WHERE id = ?');
                $req->execute(array($titre, $date, $description, $code_color, $img_illustration, $getid));
                header('Location: modifier_album.php');
            } else {
...
<?php include('include/header.php'); ?>
    <?php while ($donnees = $req->fetch()): ?>
        <div class="container mt-4">
            ...
            <a href="?id=<?= $donnees['id']; ?>" class="btn btn-primary">Modifier cet album</a>
        </div>
        <hr>
    <?php endwhile; ?>
